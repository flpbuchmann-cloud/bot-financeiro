...
retorno_3meses = acumulado_periodo(returns, 63)  # Aproximadamente 3 meses (63 dias úteis)
retorno_12meses = acumulado_periodo(returns, 252) # Aproximadamente 12 meses (252 dias úteis)
retorno_24meses = acumulado_periodo(returns, 504) # Aproximadamente 24 meses (504 dias úteis)
...
# Calcula a volatilidade anualizada de 12 meses (252 dias úteis)
volatilidade_12meses = returns.rolling(window=252).std() * np.sqrt(252)
# Calcula a volatilidade anualizada de 24 meses (504 dias úteis)
volatilidade_24meses = returns.rolling(window=504).std() * np.sqrt(252)

tabela = pd.DataFrame({
    'Ticker': tickers,
    'Retorno Acumulado no Mês': monthly_returns.iloc[-1].values,
...
for ticker in tickers:
    ...
    metrics_list.append({
        'Ticker': ticker,
        'Retorno Acumulado no Mês': monthly_ret,
...
retorno_3meses = acumulado_periodo(returns_cleaned, 63)  # Aproximadamente 3 meses (63 dias úteis)
retorno_12meses = acumulado_periodo(returns_cleaned, 252) # Aproximadamente 12 meses (252 dias úteis)
retorno_24meses = acumulado_periodo(returns_cleaned, 504) # Aproximadamente 24 meses (504 dias úteis)
...
tabela = pd.DataFrame({
    ...
    'Retorno Acumulado no Mês': monthly_returns_cleaned.iloc[-1],
    ...
    'Preço de Fechamento': last_close_prices
})
...
if SPREADSHEET_URL == 'INSIRA_A_URL_DA_SUA_PLANILHA_AQUI':
    print("⚠️ ATENÇÃO: Você precisa substituir o texto 'INSIRA_A_URL_DA_SUA_PLANILHA_AQUI' pela URL real da sua planilha na linha acima! ⚠️")
else:
    try:
        ...
        # Limpa o conteúdo existente na planilha (opcional)
        # worksheet.clear()
        ...
        print("Erro: Planilha não encontrada. Verifique se a URL está correta e se a conta de serviço tem acesso.")
...
1.  Crie uma conta no [GitHub](https://github.com/) e um novo repositório (ex: `bot-financeiro`).
2.  Faça o upload dos arquivos `financeiro.py` e `requirements.txt` para lá.

### 3. Configurar a Chave Secreta (IMPORTANTE)
Não envie seu arquivo JSON (`minhachavejson.json`) para o repositório público! Use as "Secrets":
1.  No seu repositório, vá em **Settings** > **Secrets and variables** > **Actions**.
2.  Clique em **New repository secret**.
3.  **Name:** `GCP_CREDENTIALS`
4.  **Secret:** Cole todo o conteúdo do seu arquivo JSON (`minhachavejson.json`) aqui.
5.  Clique em **Add secret**.

### 4. Ajustar o Script Python
Edite seu arquivo `financeiro.py` para ler a chave da variável de ambiente em vez do arquivo:
```python
import os
import json
import gspread
...
1.  No seu repositório, clique em **Actions** > **New workflow** > **set up a workflow yourself**.
2.  Nomeie o arquivo como `.github/workflows/execucao_diaria.yml`.
3.  Cole o conteúdo abaixo (Atenção ao horário: 08:30 BRT = 11:30 UTC):
...
on:
  schedule:
    # Roda às 11:30 UTC (08:30 Brasília) de Segunda a Sexta (1-5)
    - cron: '30 11 * * 1-5'
...
Pronto! O GitHub vai rodar seu script automaticamente nos horários definidos.
"""

# Cria o arquivo requirements.txt necessário para o GitHub Actions
content = """
yfinance
pandas
numpy
...
print("Arquivo 'requirements.txt' criado com sucesso!\nVerifique na aba 'Arquivos' (ícone de pasta à esquerda) e faça o download.")

"""# 🕒 Como Agendar a Execução Automática

O Google Colab não executa sozinho com o navegador fechado. Para rodar este script automaticamente (ex: Seg-Sex às 08:30), siga estes passos para rodá-lo no seu computador:

### Passo 1: Preparar o Script
1. Vá no menu **Arquivo** > **Fazer Download** > **Fazer Download do .py**.
...
### Passo 2: Instalar Python e Bibliotecas (se não tiver)
...
3. Dê um nome (ex: "Atualizar Planilha Financeira").
...
6. Defina o horário para 08:30.

### Opção Avançada (Nuvem)
Se preferir não deixar seu PC ligado, você pode usar serviços como **GitHub Actions** (gratuito com limites) ou **PythonAnywhere** (pago para agendamento, mas muito fácil de usar).
"""